...
def parse_data(ij):
    ...
    workbook = openpyxl.load_workbook("车辆使用申请单（市内）.xlsx")
    worksheet = workbook.active

    worksheet["A2"] = f"编号 {data_mumber}                            申请日期：   {year}年  {mouth}月  {day}日"
    ...
    hc_place = ij["回车地点2"] if str(ij["回车地点2"])  != 'nan' else ''
    ...
    worksheet["D13"] = ""
    worksheet["B15"] = f'驾 驶 员：{ij["驾驶人2"]}' if str(ij["驾驶人2"]) != 'nan' else ''
    ...
    worksheet["B16"] = f'出发时间：{ij["出车时间"]}' if str(ij["驾驶人2"]) != 'nan' else ''
    ...
    worksheet["B19"] = f'回场时间：{ij["回车时间"]}'  if str(ij["回车时间"]) != 'nan' else ''
    worksheet["B22"] = f'驾驶员签字: {ij["驾驶人2"]}' if str(ij["驾驶人2"]) != 'nan' else ''
...
if  __name__ == "__main__":
    try:
        ...
        records = pandas.read_excel("导出表格.xlsx", header=2).to_dict(orient='records')
        for ij in records[:]:
            try:
                print(ij)
                parse_data(ij)
